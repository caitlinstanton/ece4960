...
StageNode::SubscribeModels()
{
	...
	// TODO - print the topic names nicely as well
        ...
        new_robot->sim_pose_pub = n_.advertise<lab3::SimPose>(mapName(SIM_POSE, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10);
        ...
        if(!n_.getParam("INTEGRATION_ERROR_MAX_X", new_robot->INTEGRATION_ERROR_MAX_X))
            new_robot->INTEGRATION_ERROR_MAX_X = 0.04;
        if(!n_.getParam("INTEGRATION_ERROR_MAX_Y", new_robot->INTEGRATION_ERROR_MAX_Y))
            new_robot->INTEGRATION_ERROR_MAX_Y = 0.04;
        ...
        if(!n_.getParam("INTEGRATION_ERROR_MAX_A", new_robot->INTEGRATION_ERROR_MAX_A))
           new_robot-> INTEGRATION_ERROR_MAX_A = 0.15;
...
StageNode::WorldCallback()
{
  if( ! ros::ok() ) {
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        Stg::Velocity original_error = robotmodel->positionmodel->GetOdomError();


        std::cerr << "sim_interval = " << dt 
                  << " | calculated sim_interval = " << dT
                  << std::endl;

        std::cerr << "vel = (" << v.x << ", " << v.y << ", " << v.a  << ")"
                  << " | calculated vel = (" << gvel.x << ", " << gvel.y << ", " << gvel.a  << ")"
                  << std::endl;
        
     

        Stg::Pose integration_error(robotmodel->INTEGRATION_ERROR_MAX_X * (drand48() - 0.5),
                                    robotmodel->INTEGRATION_ERROR_MAX_Y * (drand48() - 0.5),
                                    0.0,
                                    robotmodel->INTEGRATION_ERROR_MAX_A * (drand48() - 0.5));


        robotmodel->positionmodel->est_pose.a = Stg::normalize(robotmodel->positionmodel->est_pose.a + ((1) * integration_error.a));
        
        robotmodel->positionmodel->est_pose.x += (1) * integration_error.x;
        robotmodel->positionmodel->est_pose.y += (1) * integration_error.y;
        ...
        std::cerr << "original error        = ( " << original_error.x << ", "
                  << original_error.y << ", "
                  << original_error.a << ")" << std::endl;


        std::cerr << "step error            = ( " << integration_error.x << ", "
                  << integration_error.y << ", "
                  << integration_error.a << ")" << std::endl;

        std::cerr << "ground truth - odom   = ( " << (gpose.x - robotmodel->positionmodel->est_pose.x) << ", "
                  << (gpose.y - robotmodel->positionmodel->est_pose.y) << ", "
                  << (gpose.a - robotmodel->positionmodel->est_pose.a) << ")" << std::endl;
        ...
        simulator_pose.odom_x = robotmodel->positionmodel->est_pose.x;
        simulator_pose.odom_y = robotmodel->positionmodel->est_pose.y;
        simulator_pose.odom_a = robotmodel->positionmodel->est_pose.a;
