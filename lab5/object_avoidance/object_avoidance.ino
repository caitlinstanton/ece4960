...
void brakeBackwards() {
  myMotorDriver.setDrive(0,1,255); //drive right motor
  myMotorDriver.setDrive(1,1,255); //drive left motor
}
...
void loop(void)
{
  ...
  {
    if((distance==0 || distance>300) && proxValue<80) //0 if there is no object nearby
    {
      forward();
    }
    else
