...
private:
    ...
    struct StageRobot
    {
        ...
        ros::Publisher sim_pose_pub; //SimPose ground truth and odom
        ...
        // Odom Params
        double INTEGRATION_ERROR_MAX_X;
        double INTEGRATION_ERROR_MAX_Y;
        double INTEGRATION_ERROR_MAX_Z;
        double INTEGRATION_ERROR_MAX_A;

    };

    std::vector<StageRobot const *> robotmodels_;

    // Used to remember estimated rboot pose
    Stg::Pose robot_odom_pose;
...
StageNode::SubscribeModels()
{
	...
	// TODO - print the topic names nicely as well
        ...
        new_robot->sim_pose_pub = n_.advertise<lab9::SimPose>(mapName(SIM_POSE, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10);
        ...
        // Get rosparams
        if(!n_.getParam("INTEGRATION_ERROR_MAX_X", new_robot->INTEGRATION_ERROR_MAX_X))
            new_robot->INTEGRATION_ERROR_MAX_X = 0.03;
        if(!n_.getParam("INTEGRATION_ERROR_MAX_Y", new_robot->INTEGRATION_ERROR_MAX_Y))
            new_robot->INTEGRATION_ERROR_MAX_Y = 0.03;
        if(!n_.getParam("INTEGRATION_ERROR_MAX_Z", new_robot->INTEGRATION_ERROR_MAX_Z))
            new_robot->INTEGRATION_ERROR_MAX_Z = 0.0;
        if(!n_.getParam("INTEGRATION_ERROR_MAX_A", new_robot->INTEGRATION_ERROR_MAX_A))
           new_robot-> INTEGRATION_ERROR_MAX_A = 0.12;
...
StageNode::WorldCallback()
{
  if( ! ros::ok() ) {
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        Stg::Velocity gdiff(0,0,0,0);
        Stg::Pose prev_gpose(0,0,0,0);
        double dT;
        if (this->base_last_globalpos.size()>r){
            Stg::Pose prevpose = this->base_last_globalpos.at(r);
            prev_gpose = this->base_last_globalpos.at(r);
            dT = (this->sim_time-this->base_last_globalpos_time).toSec();
            if (dT>0)
                gvel = Stg::Velocity(
                ...
                gdiff = Stg::Velocity(
                            ...
                            (gpose.y - prevpose.y),
                            (gpose.z - prevpose.z),
                            Stg::normalize(gpose.a - prevpose.a)
                            );
        ...
        simulator_pose.gt_x = gpose.x;
        simulator_pose.gt_y = gpose.y;
        simulator_pose.gt_a = gpose.a;
        ...
        const double dt = this->world->sim_interval / 1e6;


        Stg::Pose IMU_error(0.015 * (drand48() - 0.5),
                            0.015 * (drand48() - 0.5),
                            0.0,
                            0.008 * (drand48() - 0.5));
        

        // Stg::Pose IMU_error(0.0, 0.0, 0.0, 0.0);

        double trans_noise = robotmodel->INTEGRATION_ERROR_MAX_X * (drand48() - 0.5);
        double rot1_noise = robotmodel->INTEGRATION_ERROR_MAX_A * (drand48() - 0.5);
        double rot2_noise = robotmodel->INTEGRATION_ERROR_MAX_A * (drand48() - 0.5);

        double trans = std::hypot(gdiff.x, gdiff.y) + trans_noise;
        double rot1 = Stg::normalize(std::atan2(gdiff.y, gdiff.x) - prev_gpose.a + rot1_noise);
        double rot2 = Stg::normalize(gpose.a - prev_gpose.a - rot1 + rot2_noise);

        robot_odom_pose.x = robot_odom_pose.x + trans*std::cos(robot_odom_pose.a + rot1) + IMU_error.x;
        robot_odom_pose.y = robot_odom_pose.y + trans*std::sin(robot_odom_pose.a + rot1) + IMU_error.y;
        robot_odom_pose.a = Stg::normalize(robot_odom_pose.a + rot1 + rot2 + IMU_error.a);

        simulator_pose.odom_x = robot_odom_pose.x;
        simulator_pose.odom_y = robot_odom_pose.y;
        simulator_pose.odom_a = robot_odom_pose.a;

        // std::cerr << "rosparam error        = ( " << robotmodel->INTEGRATION_ERROR_MAX_X << ", "
        //           << robotmodel->INTEGRATION_ERROR_MAX_Y << ", "
        //           << robotmodel->INTEGRATION_ERROR_MAX_A << ")" << std::endl;
        
        // std::cerr << "trans_noise = " << trans_noise 
        //           << " | rot1_noise = " << rot1_noise  << " | rot2_noise = " << rot2_noise
        //           << std::endl;

        // std::cerr << "ground truth - odom   = ( " << (gpose.x - simulator_pose.odom_x) << ", "
        //           << (gpose.y - simulator_pose.odom_y) << ", "
        //           << (gpose.a - simulator_pose.odom_a) << ")" << std::endl;
        // new odom
        ...
        robotmodel->sim_pose_pub.publish(simulator_pose);
