...
import time
import numpy as np
from lab7.msg import PlotPoint, SimPose
import math
import sys
from std_srvs.srv import Empty, EmptyRequest, EmptyResponse
from copy import deepcopy
from loc_utils import Pose, get_max, get_sim_model, is_world_file_valid, is_map_valid
from lab7.srv import ProbDist, ProbDistRequest, ProbDistResponse
from lab7.srv import MapInit, MapInitRequest, MapInitResponse
...
class BaseRobot():
    ...
    def __init__(self):
        ...
        rospy.init_node('lab7_robot_controller', anonymous=True)
...
class VirtualRobot(BaseRobot):
    ...
    # Execute the rotation behaviour to measure observations
    def perform_observation_loop(self, observation_count, rot_vel):
        ...
        for i in range(0, len(obs_bearing_data)):
            obs_bearing_data[i] = rot_vel*loop_rot_duration*i
...
class Mapper():
    ...
    # Convert x, y, a from the continous world frame to the grid (m, m, deg)
...
class BaseLocalization():
    ...
    # Print belief stattistics and plot data in the plotter
...
class Plotter():
    ...
    def reset_plot(self):
        rospy.wait_for_service('plot_reset', timeout=3)
        try:
    ...
    # Reset Map in plotter
    def reset_map(self):
        rospy.wait_for_service('map_reset', timeout=3)
        try:
    ...
    # Plot a 2D prob distribution with a serialized
    # data of size equal to number of total grid cells
    def plot_prob_dist(self, data):
        rospy.wait_for_service('plot_prob_dist', timeout=3)
        try:
    ...
    # the lines variable in the mapper
    def visualize_map(self):
        rospy.wait_for_service('map_init', timeout=3)
        try:
