...
class Localization():
    ...
    def __init__(self, robot, mapper, sensor_sigma=0.11, odom_trans_sigma=0.33, odom_rot_sigma=15, async_op=False):
        self.robot = robot
        self.mapper = mapper
        ...
        if async_op:
            self.get_observation_data = self.__async_observation_data
        else:
            self.get_observation_data = self.__sync_observation_data
    ...
    def __sync_observation_data(self, observation_count=18, rot_vel=30):
        self.obs_range_data = self.robot.perform_observation_loop(observation_count,
                                                                  rot_vel)
        
    async def __async_observation_data(self, observation_count=18, rot_vel=30):
    ...
    def compute_control(self, cur_pose, prev_pose):
        # Angles in pose and prev_pose are in degrees
    ...
    def odom_motion_model(self, cur_pose, prev_pose, u):
        ...
        prob = (self.gaussian(u[0], true_delta_rot_1, self.odom_rot_sigma)
                * self.gaussian(u[1], true_delta_trans, self.odom_trans_sigma)
                * self.gaussian(u[2], true_delta_rot_2, self.odom_rot_sigma))
    ...
    def prediction_step(self, cur_odom, prev_odom):
        ...
        for prev_x in range(0, self.mapper.MAX_CELLS_X):
            for prev_y in range(0, self.mapper.MAX_CELLS_Y):
                for prev_a in range(0, self.mapper.MAX_CELLS_A):
                    if(self.bel[prev_x, prev_y, prev_a] > 0.00014):
        ...
        print("Uniform Belief with each cell value: ", self.bel_bar[0,0,0])
    ...
    def update_step(self):
        ...
        for i in range(0, self.mapper.OBS_PER_CELL):
            self.bel = self.bel * self.gaussian(self.mapper.obs_views[:, :, :, i] - self.obs_range_data[i],
                                                0,
                                                self.sensor_sigma)
