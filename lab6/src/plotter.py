...
from lab6.msg import PlotPoint
from lab6.msg import MapPoints
...
class GraphDrawer():
    def __init__(self, graph, symbol, symbol_size, map_line_size):
        ...
        self.plots = []
        self.plots.append(self.graph.plot([], [], pen=None, symbolPen='r',
                                          symbolBrush='r', symbolSize=self.symbol_size,
                                          symbol=self.symbol))
        self.plots.append(self.graph.plot([], [], pen=None, symbolPen='g',
                                          symbolBrush='g', symbolSize=self.symbol_size,
                                          symbol=self.symbol))
        self.plots.append(self.graph.plot([], [], pen=None, symbolPen='b',
                                          symbolBrush='g', symbolSize=self.symbol_size,
                                          symbol=self.symbol))
...
class Traj_Plotter():
    ...
    def map_init(self, data):
        if((len(data.sx) == len(data.ex)) and (len(data.sx) == len(data.sy)) and (len(data.sx) == len(data.ey))):
    ...
    def node_setup(self, symbol, symbol_size, map_line_size):
        ...
        rospy.Subscriber("/map_points", MapPoints,
                         self.map_init, queue_size=300)
...
if __name__ == '__main__':
    ...
    config_parser.read("/home/artemis/catkin_ws/src/lab6/scripts/plotter_config.ini")

    config_symbol_size = config_parser.getint('PLOTTERCONFIG', 'symbol_size')
    config_position_tolerance = config_parser.getfloat('PLOTTERCONFIG', 'position_tolerance')
