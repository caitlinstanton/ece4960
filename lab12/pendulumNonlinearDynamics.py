...
class Pendulum:
    ...
    def cartpendfunc(self, x, u):
        ...
        ) + self.m1 * self.ell * self.ell * (1.0 / D) * u
        ydot2 = thetadot
        ydot3 = (1.0 / D) * (
            (self.m1 + self.m2) * self.m1 * self.g * self.ell * Sy -
            self.m1 * self.ell * Cy *
            (self.m1 * self.ell * thetadot * thetadot * Sy - self.b * zdot)
        ) - self.m1 * self.ell * Cy * (1.0 / D) * u
        dydt = [ydot0, ydot1, ydot2, ydot3]
        #with process noise:
        dydt = [
            ydot0 + np.random.randn() * 0.01, ydot1 + np.random.randn() * 0.01,
            ydot2 + np.random.randn() * 0.01, ydot3 + np.random.randn() * 0.01
        ]
        return dydt
