...
class pendulumAn:
    ...
    def __init__(self):
        ...
        plt.axis([-3 * P.ell, 3 * P.ell, -0.1,
                  3 * P.ell])  # Change the x,y axis limits
    ...
    def drawPendulum(self, state_input):
        ...
        theta = state_input[2] + np.pi  # Angle of pendulum [radians]

        self.drawCart(z)
        self.drawCircle(z, theta)
        self.drawRod(z, theta)
        self.ax.axis(
            'equal')  # This will ensure that the image does not distort
    ...
    def drawCart(self, z):
        ...
        if self.flagInit == True:
            ...
            # to the handle list
            self.handle.append(
                mpatches.Rectangle(xy, P.w, P.h, fc='blue', ec='black'))
    ...
    def drawRod(self, z, theta):
        ...
        Y = [(P.gap + P.h),
             (P.gap + P.h) - P.ell * np.cos(theta[0])]  # Y data points
    ...
    def drawCircle(self, z, theta):
        ...
        y = (P.gap +
             P.h) - (P.ell + P.radius) * np.cos(theta[0])  # y coordinate
